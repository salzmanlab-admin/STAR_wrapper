...
def modify_class(out_path, name, assembly, dep = ""):
  ...
  sbatch_file("run_modify_class.sh",out_path, name, "modify_{}".format(name), "12:00:00", "300Gb", command, dep=dep)
...
def GLM(out_path, name, single, dep = ""):
  ...
  if single:
    ...
    command += " 0 "
  sbatch_file("run_GLM.sh", out_path, name,"GLM_{}".format(name), "12:00:00", "300Gb", command, dep=dep)
...
def ann_SJ(out_path, name, assembly, gtf_file, single, dep = ""):
  ...
  if single:
    command += "--single "
  sbatch_file("run_ann_SJ.sh", out_path, name,"ann_SJ_{}".format(name), "24:00:00", "70Gb", command, dep=dep)
