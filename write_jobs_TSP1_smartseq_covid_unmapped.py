...
def whitelist(data_path,out_path, name, bc_pattern, r_begins):
  command = "mkdir -p {}{}\n".format(out_path, name)
  command += "umi_tools whitelist "
  command += "--stdin {}{}{} ".format(data_path, r_begins[0], name)
...
def extract(out_path, data_path, name, bc_pattern, r_begins, dep = ""):
  command = "umi_tools extract "
  command += "--bc-pattern={} ".format(bc_pattern)
  command += "--stdin {}{}{} ".format(data_path, r_begins[0], name)
  command += "--stdout {}{}_extracted{} ".format(data_path, name, r_begins[0], name)
  command += "--read2-in {}{}{} ".format(data_path, name, r_beginss[1])
  command += "--read2-out={}{}_extracted{} ".format(data_path, name, r_beginss[1], name)
...
def STAR_map(out_path, data_path, name, r_begins, star_index, gzip, cSM, cJOM, aSJMN, cSRGM, sIO, sIB, single, gtf_file, tenX, dep = ""):
  ...
  for i in range(l,2):
    ...
    command += "--genomeDir {} ".format(star_index)
    if tenX:
      command += "--readFilesIn {}{}_extracted{} ".format(data_path, r_begins[i], name)
    else:
      command += "--readFilesIn {}{}{} ".format(data_path, r_begins[i], name)
    if gzip:
...
# TSP1 10x sample
  data_path = "/oak/stanford/groups/horence/Roozbeh/single_cell_project/output/TS_pilot_smartseq_cSM_10_cJOM_10_aSJMN_0_cSRGM_0/"+args.sample+"/"
  assembly = "hg38_EBV"
  star_index = "/scratch/PI/horence/rob/data/ebv/ebv_only_star_ind_2.7.3a"
  run_name = "TS_pilot_smartseq_ebv_only/"+args.sample
  r_begins = ["1", "2"]
  names = ["Unmapped.out.mate1"]
  gtf_file = "/oak/stanford/groups/horence/Roozbeh/single_cell_project/utility_files/chrEBV_Akata_inverted.gtf"
  single = False
  tenX = False
  HISAT = False
            ...
            for sIB in scoreInsBase:
              for sDO in scoreDelOpen:
                for sDB in scoreDelBase:
                  out_path = "/scratch/PI/horence/Roozbeh/single_cell_project/output/{}/".format(run_name)
                  ...
                  for name in names:
                    ...
                    if run_whitelist or curr_run_whitelist:
                      whitelist_jobid = whitelist(data_path,out_path, name, bc_pattern, r_begins)
                      jobs.append("whitelist_{}.{}".format(name, whitelist_jobid))
                      job_nums.append(whitelist_jobid)
                    else:
                      whitelist_jobid = ""
                    if run_extract or curr_run_extract:
                      extract_jobid = extract(out_path, data_path, name, bc_pattern, r_begins, dep = ":".join(job_nums))
                      jobs.append("extract_{}.{}".format(name, extract_jobid))
                      job_nums.append(extract_jobid)
                    else:
                      extract_jobid = ""
                    if run_map:
                      map_jobid = STAR_map(out_path, data_path, name, r_begins, star_index, gzip, cSM, cJOM, aSJMN, cSRGM, sIO, sIB, single, gtf_file, tenX, dep = ":".join(job_nums))
                      jobs.append("map_{}.{}".format(name,map_jobid))
                      job_nums.append(map_jobid)

                    if run_HISAT_map:
                      HISAT_map_jobid = HISAT_map(out_path, data_path, name, r_begins, assembly, single, tenX, dep = ":".join(job_nums))
