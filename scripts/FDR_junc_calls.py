...
def get_score_df(results, data_path, col, use_cols, score_df, hard_filt_df, unlim_read, frac_genomic, sd_overlap, avg_AT, avg_ent, intron_len, avg_GC):
  t0 = time.time()
...
def write_files(median_df, col, data_path, dp_dict,unlim_read, value, frac_genomic, sd_overlap, avg_AT, avg_ent, intron_len, avg_GC):
#  if unlim_read:
#    suffix = "_unlim_read"
#  else:
#    suffix = ""
...
def main():
  ...
  for data_path in args.data_paths:
    score_df, hard_filt_df = get_score_df(dp_dict[data_path], data_path, col, use_cols, score_df, hard_filt_df, args.unlim_read, args.frac_genomic, args.sd_overlap, args.avg_AT, args.avg_ent, args.intron_len, args.avg_GC)
...
#median_df.to_csv("{}{}_median{}_{}.tsv".format(data_path,col,suffix,value),sep="\t")
  ...
  for data_path in data_paths:
    ...
    write_files(median_df, col, data_path, dp_dict, args.unlim_read, args.value, args.frac_genomic, args.sd_overlap, args.avg_AT, args.avg_ent, args.intron_len, args.avg_GC)
    print("wrote files 1")
    suffix = write_files(hard_median_df, "hard_inc", data_path, dp_dict, args.unlim_read, 1, args.frac_genomic, args.sd_overlap, args.avg_AT, args.avg_ent, args.intron_len, args.avg_GC)
