...
compute_junc_cdf <- function(class_input, p_predicted_column, per_read_column, junc_cdf_column,is.10X){
  ...
  if(is.10X == 0){
    mu_i = mean(log( (1-class_input[fileTypeR1 == "Aligned"| (fileTypeR1 == "Chimeric" & readClassR1 == "lin")]$per_read_prob)/ class_input[fileTypeR1 == "Aligned"| (fileTypeR1 == "Chimeric" & readClassR1 == "lin")]$per_read_prob) )
    var_i = var(log( (1-class_input[fileTypeR1 == "Aligned"| (fileTypeR1 == "Chimeric" & readClassR1 == "lin")]$per_read_prob)/ class_input[fileTypeR1 == "Aligned"| (fileTypeR1 == "Chimeric" & readClassR1 == "lin")]$per_read_prob) )
    all_per_read_probs = class_input[(fileTypeR1 == "Aligned") | (fileTypeR1 == "Chimeric" & readClassR1 == "lin")]$per_read_prob
  } else{
...
#class_input =  fread("G:\\Shared drives\\Salzman Lab Team Drive\\Members\\Roozbeh\\Projects\\Tabula-Sapiens\\class_input_files\\HISAT\\HISAT_mismatch\\class_input_WithinBAM_benchmark_OL1.tsv", nrows = 500000, sep = "\t", header = TRUE)
...
class_input =  fread(paste(directory, "class_input.tsv", sep = ""), sep = "\t", header = TRUE)
###############################################
...
junction_prediction[, c("flagR1A","flagR1B","cigarR1A","cigarR1B","readoverhang1_length","readoverhang2_length"):= NULL]

write.table(junction_prediction, paste(directory,"GLM_output_light.txt", sep = ""), row.names = FALSE, quote = FALSE, sep = "\t")
write.table(class_input, paste(directory,"class_input_GLM.tsv", sep = ""), row.names = FALSE, quote = FALSE, sep = "\t")
