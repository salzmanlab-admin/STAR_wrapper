...
def STAR_map(out_path, data_path, name, r_ends, star_index, gzip, cSM, cJOM, aSJMN, cSRGM, sIO, sIB, single, gtf_file, tenX, dep = ""):
...
# TSP1 10x sample
  ...
  folder = path.split("/")[0]
  sample = path.split("/")[1]
  data_path = "/oak/stanford/groups/horence/Roozbeh/single_cell_project/output/TS_pilot_10X_withinbam_cSM_10_cJOM_10_aSJMN_0_cSRGM_0/"+sample+"/"
  assembly = "hg38_EBV"
  star_index = "/scratch/PI/horence/rob/data/ebv/ebv_only_star_ind_2.7.3a"
  run_name = "TS_pilot_10X_ebv_only/"+sample
  r_ends = ["_R1_001.fastq.gz", "mate1"]
  names = ["2Unmapped.out."]
              ...
              #cond_run_name = run_name + "_cSM_{}_cJOM_{}_aSJMN_{}_cSRGM_{}_sIO_{}_sIB_{}".format(cSM, cJOM, aSJMN, cSRGM, sIO, sIB)
                  ...
                  for name in names:
                    ...
                    if run_extract or curr_run_extract:
                      ...
                      extract_jobid = ""
                    if run_map:
                      map_jobid = STAR_map(out_path, data_path, name, r_ends, star_index, gzip, cSM, cJOM, aSJMN, cSRGM, sIO, sIB, single, gtf_file, tenX, dep = ":".join(job_nums))
